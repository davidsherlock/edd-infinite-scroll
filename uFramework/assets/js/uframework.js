...
(function( $ ) {
    ...
    $('body').on('change', '.uframework-shortcode-generator [data-shortcode-attr], .uframework-shortcode-generator [data-shortcode-attr] input, .uframework-shortcode-generator [data-shortcode-content]', function() {
        ...
        container.find('[data-shortcode-attr]').each(function() {
            ...
            // Filter by uFramework_shortcode_attr_{attr}
            override = wp.hooks.applyFilters( 'uFramework_shortcode_attr_' + $(this).data('shortcode-attr'), override, $(this) );
            ...
            // Skip attr if empty
            if( $(this).data('clear-if-empty') === true ) {
                if( $(this).hasClass('cmb2-list') ) {
            ...
            // Skip attr if hidden
            if( $(this).data('clear-if-hidden') === true && ! $(this).is(':visible') ) {
                return;
            }
            ...
            } else if( $(this).hasClass('cmb2-list') ) {
                if( $(this).find('input:checked').length ) {
                    var values = [];
        ...
        if( container.find('[data-shortcode-content="true"]').length ) {
            shortcode += container.find('[data-shortcode-content="true"]').val();
            shortcode += '[/' + shortcode_input.data('shortcode') + ']';
        }
